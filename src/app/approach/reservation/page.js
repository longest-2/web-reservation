...
const GoodsPriceContainer = styled.div`
	...
	${({ isInfo }) =>
		isInfo
			? `background:linear-gradient(106.88deg, rgba(65, 50, 42, 0.05) 0%, rgba(40, 48, 129, 0.2) 100%); color: ${primaryBlue}`
...
const dummyGoods = [
	{ label: "2시간(즈증)", price: "15000" },
	{ label: "3시간(주중)", price: "30000" },
	{ label: "종일권", price: "70000" },
];
	...
	return (
		<>
			...
			>
				...
				<Stack flex flexDirection="row" columnGap={2}>
					{selectedDateId !== -1 &&
						dateTable[selectedDateId].goods.map((item) => {
							return (
								<Stack
									key={item.id}
									sx={{
